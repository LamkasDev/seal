...
func NewVulkanMeshBuffer(device *logical.VulkanLogicalDevice, options VulkanMeshBufferOptions) (VulkanMeshBuffer, error) {
	...
	if vertexBuffer.StagingBuffer, err = NewVulkanBuffer(device, GetVulkanMeshBufferOptionsSize(&options), vulkan.BufferUsageFlags(vulkan.BufferUsageTransferSrcBit), vulkan.SharingModeExclusive, vulkan.MemoryPropertyFlags(vulkan.MemoryPropertyHostVisibleBit|vulkan.MemoryPropertyHostCoherentBit)); err != nil {
	...
	if err := CopyVulkanMeshBuffer(&vertexBuffer); err != nil {
		return vertexBuffer, err
	}
...
func CopyVulkanMeshBuffer(buffer *VulkanMeshBuffer) error {
	...
	if res := vulkan.MapMemory(buffer.Device.Handle, buffer.StagingBuffer.Memory, GetVulkanMeshBufferOptionsUniformsOffset(&buffer.Options), GetVulkanMeshBufferOptionsUniformsSize(&buffer.Options), 0, &vulkanIndexData); res != vulkan.Success {
		logger.DefaultLogger.Error(vulkan.Error(res))
		return vulkan.Error(res)
	}
